...
#The following 7 lines of code configures system lib (libFile) to load
...
temp = {'Windows':'NLPIR%s.dll', 'Linux':'libNLPIR%s.so'}.get(platform.system(),None)
if temp is None: raise RuntimeError('Unknow system platform!')
libFile = base + (temp % bits)  #Lib File should be among(NLPIR64.dll, NLPIR32.dll, libNLPIR64.so, libNLPIR32.so)
...
def translatePOS(sPOS):
    if sPOS=='url': sPOS = 'xu' #fix the url bug
    c = sPOS[0]
    POS_1 = POS.get(c,None)
    POS_2 = POS_1.get(sPOS,c) if POS_1 is not None else c
    return POS_2
...
if not Init(base,ENCODING.UTF8_CODE,''):     #use this line for python 2.x
